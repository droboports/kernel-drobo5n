...
struct inode *notifyfs_iget(struct super_block *sb, struct inode *lower_inode) {
	...
	notifyfs_set_lower_inode(inode, lower_inode);
...
/*
 * Connect a notifyfs inode dentry/inode with several lower ones.  This is
 * the classic stackable file system "vnode interposition" action.
 *
 * @dentry: notifyfs's dentry which interposes on lower one
 * @sb: notifyfs's super_block
...
int notifyfs_interpose(struct dentry *dentry, struct super_block *sb,
	...
	/*
	 * We allocate our new inode below by calling notifyfs_iget,
	 * which will initialize some of the new inode's fields
	 */

	/* inherit lower inode number for notifyfs's inode */
	inode = notifyfs_iget(sb, lower_inode);
	if (IS_ERR(inode)) {
		err = PTR_ERR(inode);
		goto out;
	}
...
/*
 * Main driver function for notifyfs's lookup.
 *
...
struct dentry *notifyfs_lookup(struct inode *dir, struct dentry *dentry,
	...
	notifyfs_get_lower_path(parent, &lower_parent_path);
