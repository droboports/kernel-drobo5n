...
static ssize_t notifyfs_read(struct file *file, char __user *buf,
...
static int notifyfs_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (willwrite && !lower_file->f_mapping->a_ops->writepage) {
		err = -EINVAL;
		printk(KERN_ERR "notifyfs: lower file system does not "
	...
	if (!NOTIFYFS_F(file)->lower_vm_ops) {
		...
		if (err) {
			printk(KERN_ERR "notifyfs: lower mmap failed %d\n", err);
			goto out;
		}
	...
	if (!NOTIFYFS_F(file)->lower_vm_ops) /* save for our ->fault */
...
static int notifyfs_open(struct inode *inode, struct file *file)
{
	...
	file->private_data =
		kzalloc(sizeof(struct notifyfs_file_info), GFP_KERNEL);
	if (!NOTIFYFS_F(file)) {
		err = -ENOMEM;
		goto out_err;
	}
