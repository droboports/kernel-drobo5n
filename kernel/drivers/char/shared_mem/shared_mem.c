...
void armadaSendDoorbell(MV_U32 cpuBitMask, MV_U32 chnId);

#define init_MUTEX(x) sema_init(x, 1)

char* region_name[] =
{"SHMQ","SHLX","SHVX","SHJ1","read","SGLD"};
#define MAX_REGION_NAME 6
...
static unsigned int  start_time[SHM_MAX_QUEUE_PAIRS], stop_time[SHM_MAX_QUEUE_PAIRS];
static shm_queue_os_info *global_queue_os_info;

static irqreturn_t shm_doorbell_isr( int irq , void *dev_id )
{
    unsigned int cause;
    int db;	
    shm_queue_os_info *queue_os_info = global_queue_os_info; //(shm_queue_os_info *) dev_id; wrong value due to per cpu

    cause = MV_REG_READ(SHM_DB_CAUSE_REG) & SHM_DB_CAUSE_REG_MASK;
    //printk(KERN_INFO "%s: doorbell irq cause 0x%0x queue_os_info %p\n", __func__, cause, dev_id);

    if (cause) 
    {
        /* Clear cause */
        MV_REG_WRITE( SHM_DB_CAUSE_REG, ~cause);
	db = 0;
	cause = (cause >> SHM_DB_CAUSE_REG_SHIFT);
	while(cause)
	{
		if(cause & (1 << db))
		{
                        //printk(KERN_INFO "%s: doorbell irq cause 0x%0x index %d\n", __func__, cause, db);
			    ...
			    (queue_os_info->db_to_q_index[db].type == BIT_SHIFT_LX_TO_VX) )
			{
			   ...
			   {
                              //printk(KERN_INFO "doorbell number %d tick 0x%x\n", db, jiffies);
...
int shm_doorbell_init(shm_queue_os_info *queue_os_info)
{
	...
	/* Mask off all doorbell interrupts to start with */
	for(i = 0; i < SHM_QUEUE_MAX_DOORBELL_BIT_SHIFT; i++)
	{
 	   //MV_REG_BIT_RESET(CPU_DOORBELL_IN_MASK_REG, (1 << (SHM_DB_CAUSE_REG_SHIFT+i)));
 	   MV_REG_BIT_RESET(SHM_DB_MASK_REG, (1 << (SHM_DB_CAUSE_REG_SHIFT+i)));
	}
        global_queue_os_info = queue_os_info;

	/* Hook up the interrupts and turn them on */
    	if( request_percpu_irq(DOORBELL_IN_IRQ, 
			shm_doorbell_isr, 
       			SHARED_MEM_NAME, 
			(void *) queue_os_info) ) 
    	{
		printk("SHARED_MEM: %s Unable to get an irq for doorbell in interrupt: 0x%lx\n",
			 __func__, (unsigned int long) DOORBELL_IN_IRQ);
        	return -1;
    	}

        /* Unmask all doorbell interrupts */
	for(i = 0; i <= SHM_QUEUE_MAX_DOORBELL_BIT_SHIFT; i++)
	{
	    //MV_REG_BIT_SET(CPU_DOORBELL_IN_MASK_REG, (1 << (SHM_DB_CAUSE_REG_SHIFT+i)));
	    MV_REG_BIT_SET(SHM_DB_MASK_REG, (1 << (SHM_DB_CAUSE_REG_SHIFT+i)));
	}
	printk("SHARED_MEM: request doorbell irq: 0x%lx dbell mask ipc: 0x%0x dbell mask drobo %x queue_os_info %p \n",
		 (unsigned int long) DOORBELL_IN_IRQ, CPU_DOORBELL_IN_MASK_REG, SHM_DB_MASK_REG, queue_os_info);
	return 0;
}
void do_shared_mem_rx_irq(int irq, struct pt_regs *regs)
{
	shm_queue_os_info *queue_os_info = &(g_shm_dev->queue_os_info);
        int db;	

        return;

/* We really should get db from ISR rather than looping. 
*  Not sure what's the imapct of wake up if the thread is not waiting to wake up.
*  I may have to save the waiting queues and wake up only those
*/
	for (db=0; db < SHM_QUEUE_MAX_DOORBELL_BIT_SHIFT; db++)
	{
		if( (queue_os_info->db_to_q_index[db].type == BIT_SHIFT_VX_TO_LX) ||
		    (queue_os_info->db_to_q_index[db].type == BIT_SHIFT_LX_TO_VX) )
		{
...
inline void shm_ipc_send_doorbell(MV_U32 cpuId, MV_U32 chnId)
{
        unsigned int cpuBitMask;
        unsigned int doorbellNum;

        cpuBitMask   = (1 << cpuId);
        doorbellNum  = SHM_DB_CAUSE_REG_SHIFT + chnId;

        MV_REG_WRITE(CPU_SW_TRIG_IRQ, ((1 << 24) | (cpuBitMask << 8) | doorbellNum));
...
int shm_region_name_match(char *tag)
{
	unsigned int i;

	for (i=0; i < MAX_REGION_NAME; i++)
	{
		if (!memcmp(region_name[i], tag, 4))
			return 0;
	}
...
void shm_queue_init(shm_queue_os_info *queue_os_info)
{
	...
	for(i = 0; i < (2 * SHM_MAX_QUEUE_PAIRS) /*SHM_QUEUE_MAX_DOORBELL_BIT_SHIFT*/; i++)
	{
...
unsigned int shm_queue_sync_cores(shared_mem_dev *shm_dev)
{
	...
	queue_os_info->queueHeader = region_offset.virtAddr;
...
{
	/*
	armadaSendDoorbell(0x02, 0); //test
	armadaSendDoorbell(0x02, 1); //test
        */
...
int shm_queue_send_msg_base(shared_mem_dev *shm_dev, SharedMemQueueSendMsg *send_args, int from_user)
{
	...
	shm_ipc_send_doorbell(SHM_DOORBELL_VXWORKS_CPU_ID, queue_pair_os_info->shmQPairPtr->qLxToVx.doorbellBitShift);
...
{
	return shm_queue_recv_msg_base(shm_dev, recv_args, 0);
}
...
{
	int retVal;
	unsigned int size, prevSlot;
	volatile SHMQueueSlotHeader *slotHeader = NULL, *prevSlotHeader;
	...
	slotHeader = (SHMQueueSlotHeader *) (queue_pair_os_info->slot_buf_ptr_vx_to_lx + 
        ...
        start_time[recv_args->handle] = jiffies;
	while(SHM_QUEUE_GET_SLOT_HEADER_OWNER(slotHeader) != SHM_QUEUE_SLOT_OWNER_LINUX)
	{
#if 0
...
//#endif
        ...
        stop_time[recv_args->handle] = jiffies;
/*
        if (start_time[recv_args->handle] != stop_time[recv_args->handle]) 
        {
		printk("SHARED_MEM: recv_msg q %d wait time %d ticks= %x\n", recv_args->handle, 
                                                                             stop_time[recv_args->handle] - start_time[recv_args->handle],
                                                                             stop_time[recv_args->handle]);
...
*/
	...
	prevSlot = (queue_pair_os_info->shmQPairPtr->qVxToLx.nextSlotToProcess > 0) ?
                   queue_pair_os_info->shmQPairPtr->qVxToLx.nextSlotToProcess - 1 :
                   queue_pair_os_info->shmQPairPtr->qVxToLx.numSlots - 1;
	...
	/* Ping the other Core in case it is waiting and the queue is full */
	//SHM_DOORBELL_SET(SHM_DOORBELL_VXWORKS_CPU_ID, queue_pair_os_info->shmQPairPtr->qVxToLx.doorbellBitShift);
	//shm_ipc_send_doorbell(SHM_DOORBELL_VXWORKS_CPU_ID, queue_pair_os_info->shmQPairPtr->qVxToLx.doorbellBitShift);

        //nextSlot = queue_pair_os_info->shmQPairPtr->qVxToLx.nextSlotToProcess;

        prevSlotHeader = (SHMQueueSlotHeader *) (queue_pair_os_info->slot_buf_ptr_vx_to_lx +
        (prevSlot*(queue_pair_os_info->shmQPairPtr->qVxToLx.sizeSlots + sizeof(SHMQueueSlotHeader))));

        if (SHM_QUEUE_GET_SLOT_HEADER_OWNER(prevSlotHeader) == SHM_QUEUE_SLOT_OWNER_LINUX)
        {
           shm_ipc_send_doorbell(SHM_DOORBELL_VXWORKS_CPU_ID,
                                 queue_pair_os_info->shmQPairPtr->qVxToLx.doorbellBitShift);
        }
...
void wait_for_init_shared_mem(void *dev)
{
	...
	if (!shared_mem_init_done)
	{
		...
		{
                       /*
                       if((shm_dev->sharedMemHeader->header_magic == SHARED_MEM_HEADER_MAGIC))
                       {
		           printk("SHARED_MEM: detected shared memory magic 0x%x\n", shm_dev->sharedMemHeader->header_magic);
                       }
                       */
...
void parse_shared_mem_header(shared_mem_dev *shm_dev, shared_mem_os_info *info)
{
  ...
  for(i = 0; i < shm_dev->sharedMemHeader->num_partitions; i++)
  {
	  ...
	  region_info->physAddr = partitionInfo->phyAddr;
	  /*
	   * Remap only Linux shared memory regions 
	   */
          if (shm_region_name_match(region_info->tag) == 0)
	  {	
	  ...
	  region_info->num_mmaps = 0x0;
          if (shm_region_name_match(region_info->tag) == 0)
	  {
             printk("SHARED_MEM: region name %s  phy_addr %x virt_addr %x size %x\n", 
                    region_info->tag, region_info->physAddr, 
		    region_info->virtKernelAddr, region_info->regionSize);
	  }
	  info->num_regions++;
  }
}
...
int find_region_kern_addr_from_tag(void *dev, char *tag, void **phys, void **virt, int **reg_size)
{
	...
	for (i = 0; i < shm_dev->os_info->num_regions; i++)
	{
		...
		{
                        *phys = (void *)shm_dev->os_info->region_info[i].physAddr;
			*virt = (void *)shm_dev->os_info->region_info[i].virtKernelAddr;
			*reg_size = (int *)shm_dev->os_info->region_info[i].regionSize;
			return 0;
		}
	}
...
int find_region_from_tag(shared_mem_os_info *os_info, SharedMemGetRegionOffset *offset)
{
	...
	for(i = 0; i < os_info->num_regions; i++)
	{
		if(!memcmp(offset->tag, os_info->region_info[i].tag, 4))
		{
			...
			offset->virtAddr = os_info->region_info[i].virtKernelAddr;
			return 0;
		}
	}
...
static long shared_mem_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
  shared_mem_dev *shm_dev = (shared_mem_dev *) file->private_data;
  int retval = 0;
...
static int shared_mem_mmap(struct file *file, struct vm_area_struct *vma)
{
  ...
  shared_mem_dev *shm_dev = (shared_mem_dev *) file->private_data;
  printk("SHARED_MEM: shared_mem_mmap. Begin\n");
...
static int shared_mem_panic_event(struct notifier_block *th, unsigned long event, void *ptr)
{
    ...
    shm_ipc_send_doorbell(SHM_DOORBELL_VXWORKS_CPU_ID, SHM_QUEUE_SPECIAL_PANIC_DB_BITSHIFT);
    return NOTIFY_DONE;
}
...
struct file_operations shared_mem_fops =
{
	...
	.readdir=	NULL,
	.poll	=	NULL,
	.unlocked_ioctl	=	shared_mem_unlocked_ioctl,
...
static int __init shared_mem_module_init(void)
{
  ...
  printk("SHARED_MEM: mapping shared memory Header physical addr %x virtual addr 0x%p\n", SHARED_MEMORY_HEADER_PHYS_ADDR, shm_dev->sharedMemHeader);
  //ret = (SharedMemPartitionDescHeader *) ioremap_nocache(SHARED_MEMORY_BASE_PHYS_ADDR, SHARED_MEM_TOTAL_SHARED_MEM);

  if(!shm_dev->sharedMemHeader)
  {
    printk("SHARED_MEM: error mapping shared memory Header into kernel memory at addr 0x%x\n", SHARED_MEMORY_HEADER_PHYS_ADDR);
    return -EIO;
  }
#endif
...
#if 0
  {
  	volatile unsigned int *mem = (volatile unsigned int *) shm_dev->sharedMemHeader;
  	*mem = 0x12345678;
...
#define BASE_DOORBELL_OFFSET 16 
void armadaSendDoorbell(MV_U32 cpuBitMask, MV_U32 chnId)
{
        MV_U32 doorbellNum;

        doorbellNum  = BASE_DOORBELL_OFFSET + chnId;
...
{
  printk(KERN_INFO "%s: Got an IPC doorbell: irq# %d\n", __func__, irq);
}
EXPORT_SYMBOL(do_ipc_rx_irq);
#endif
