 ...
 */
static int notifyfs_remount_fs(struct super_block *sb, int *flags, char *options)
{
	int err = 0;
	...
	if ((*flags & ~(MS_RDONLY | MS_MANDLOCK | MS_SILENT)) != 0) {
		printk(KERN_ERR
		       "notifyfs: remount flags 0x%x unsupported\n", *flags);
		err = -EINVAL;
	}
...
int notifyfs_init_inode_cache(void)
{
	...
	notifyfs_inode_cachep =
		kmem_cache_create("notifyfs_inode_cache",
